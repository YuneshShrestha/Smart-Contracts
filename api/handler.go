...
func NewHandler(auction *contract.Contract) *Handler {
    return &Handler{
        auction: auction,
    }
}

// txOpts returns a bind.TransactOpts struct and an error.
// It takes a *gin.Context as input and extracts the private key from the request header.
// The private key is then used to create a new bind.TransactOpts struct with the specified chain ID.
func txOpts(c *gin.Context) (*bind.TransactOpts, error) {
    // privateKey, err := crypto.HexToECDSA(c.Request.Header["key"][0])

    privateKey, err := crypto.HexToECDSA(c.Request.Header[http.CanonicalHeaderKey("key")][0])
    if err != nil {
        return nil, err
    }
